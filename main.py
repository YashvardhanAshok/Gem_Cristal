...
def gem_find(driver,gem_ids,element):
...
def gem_funtion(threading_filename ,json_dir ,ministry_name ,elements_list):
    ...
    for element in elements_list:
        ...
        try:
            for page_no in range(int(max_page)):
                # Gem-funtion
                extracted_data.append(gem_find(driver,gem_ids,element))
...
def Main():
    try:
        ...
        for elements in item_list: 
            ...
            t = threading.Thread(target=gem_funtion, args=(threading_filename,json_dir ,ministry_name,elements))
            t.start()
            threads.append(t)
            count = count  + 1 
            if count == 5:
