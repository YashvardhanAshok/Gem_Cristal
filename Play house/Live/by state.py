...
def gem_find(driver, card_elements, card, gem_ids, close_tender_id_list,gem_ids_copy,state):
    global all_gem_ids
    # scroll
    ...
    try:
        ...
        #     except: pass
        #     print(f"gem id skipped: {state}, {bid_title.text} and started at: {start_date}")
        #     return 
        # elif bid_title.text in close_tender_id_list:
        #     print(f"--xx gem id {state}, {bid_title.text} extended xx--")
        #     return {"extended": today.strftime("%d-%b-%Y"),"DATE OF SEARCH": today.strftime("%d-%b-%Y"),"TENDER ID": bid_title.text,"END DATE": end_date,"END Time": end_date_time,"state":state}
        # elif bid_title.text in all_gem_ids:
        #     print(f"--xx gem id {state}, {bid_title.text} in db xx--")
        #     return {"extended": today.strftime("%d-%b-%Y"),"DATE OF SEARCH": today.strftime("%d-%b-%Y"),"TENDER ID": bid_title.text,"END DATE": end_date,"END Time": end_date_time,"state":state}
        ...
        try:
            ...
            if os.path.exists(download_path): 
                print(f"have file for:{state}, {bid_id_no}")
                
                # try:
                #     if bid_title.text in all_gem_ids:
                #         all_gem_ids.append(bid_title.text)
                #         print(f"alrady in db:{state}, {bid_title.text}")
                #         return 
                # except: pass
    ...
    try:
        while True:
            ...
            for card in card_elements:
                ...
                try:
                    json_data = gem_find(driver, card_elements, card, gem_ids, close_tender_id_list,gem_ids_copy,state)
...
def gem():
    try:
        ...
        state_list = ["MANIPUR","ASSAM","MEGHALAYA","SIKKIM","TRIPURA","ARUNACHAL","MIZORAM","NAGALAND"]
        # state_list = ["MANIPUR"]
