...
query = """
    ...
    AND organisation not IN ('ASSAM RIFLES');
"""
...
for _, row in df.iterrows():
    ...
    try:
        ...
        if isinstance(placeholder_data, list) and len(placeholder_data) > 0:
            entry = placeholder_data[0]  # Only L1
...
# Generate SN and Count
grouped = final_df.groupby('Name').agg(
    Count=('Tender_id', 'count'),
    Total_Amount=('Amount', 'sum')
).reset_index()
grouped.insert(0, 'Sn', grouped.index + 1)

# Merge SN and Count back to main data
final_df = final_df.merge(grouped[['Name', 'Sn', 'Count']], on='Name', how='left')
final_df.sort_values(by=['Sn', 'Name', 'Tender_id'], inplace=True)

# Insert total rows per company
summary_rows = []
for name, group in final_df.groupby('Name'):
    ...
    summary_rows.append(pd.DataFrame([total_row]))

main_sheet_df = pd.concat(summary_rows, ignore_index=True)

# Reorder final columns
main_sheet_df = main_sheet_df[['Sn', 'Count', 'Name', 'L_Label', 'Tender_id', 'Department', 'Item_description', 'qty',
                               'address', 'Start_date', 'End_date', 'Tender_value', 'Amount']]

# Write both sheets to Excel
output_path = "N_as_Only_L1_Bidders_with_Summary.xlsx"
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    grouped[['Sn', 'Name', 'Count', 'Total_Amount']].to_excel(writer, sheet_name='Summary', index=False)
    main_sheet_df.to_excel(writer, sheet_name='L1 Data', index=False)

# Load workbook to apply formatting
wb = load_workbook(output_path)
ws = wb['L1 Data']

# Alternating row fill for L1 Data
...
# Save workbook
wb.save(output_path)
print(f"Excel file saved with Summary and L1 Data: {output_path}")
