                ...
                for gem_id in gem_ids:
                    if bid_title.text == gem_id[0]:
                        if gem_id[1][0] == "Yes":
                            ...
                            gem_ids.pop(count)
                            print("gem id extended:", bid_title.text)
                            event_data = {
                                "extended": today.strftime("%d-%b-%Y"),
...
def sql(extracted_data, gem_ids):
    with db_lock:
        conn = pyodbc.connect(
        ...
        for tender_data in extracted_data:
            ...
            # Default 'extended' to current date unless overridden later
            extended = date_of_search

            if exists:
                update_sql = """
                    UPDATE tender_data
                    SET end_date = ?, end_time = ?, date_of_search = ?, extended = ?
                    WHERE tender_id = ?
                """
                cursor.execute(update_sql, (end_date, end_time, date_of_search, extended, tender_id))
                conn.commit()
        ...
        # Handle gem_ids for cancellation count logic
        if gem_ids and isinstance(gem_ids, list) and len(gem_ids) > 1 and isinstance(gem_ids[1], tuple):
            if gem_ids[1][0] != 'No':
                cancel = 1 + gem_ids[1][1]
                if cancel < 5:
                    for tender_data in extracted_data:
                        tender_id = tender_data["TENDER ID"]
                        ...
                        if exists:
                            update_sql = """
                                ...
                                WHERE tender_id = ?
                            """
                            cursor.execute(update_sql, (cancel, tender_id))
                            conn.commit()
                            print(f"xxx Tender ID {tender_id} cancel updated to {cancel} xxx")

        cursor.close()
        conn.close()

def gem_funtion(ministry_name, Organization_name,pre_tender):
    ...
    for org_name in Organization_name:
        for d_tender in pre_tender:
            if org_name == d_tender[0]:
                gem_ids = d_tender[1]
                # print("gem id",gem_ids)
                break
            else:
                gem_ids = [['GME/2125/B/6185022', ['No', '0']]]
        extracted_data = []
        ...
        for MINISTRY in MINISTRY_list: 
            ...
            for department, group in df.groupby('department'):
                ...
                for _, row in group.iterrows():
                    tenders.append([row['tender_id'], [row['Live'], row['Cancel']]])
                pre_tender.append([department, tenders])
            while True:
            ...
            t = threading.Thread(target=gem_funtion, args=(ministry_name,Organization_name,pre_tender))
            t.start()
            threads.append(t)
